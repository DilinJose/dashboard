...
const InventorySlice = createSlice({
    ...
    reducers: {
        searchInventory: (state, action) => {
            state.inventoryData = state.inventoryData.filter(inventory => inventory.name.toLowerCase().includes(action.payload.toLowerCase()) || inventory.code.includes(action.payload));
